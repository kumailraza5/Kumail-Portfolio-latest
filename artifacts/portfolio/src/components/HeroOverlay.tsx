...
export function HeroOverlay({ isMobile = false, inline = false }: HeroOverlayProps) {
  ...
  useEffect(() => {
    gsap.fromTo(
      [badgeRef.current, nameRef.current, titleRef.current, tagsRef.current, ctaRef.current],
      { opacity: 0, y: 18 },
  ...
  if (isMobile && inline) {
    return (
      <div className="flex flex-col items-center text-center w-full">
        ...
        <h1
          ref={nameRef}
          className="text-[2.6rem] font-black tracking-tight text-white leading-none mb-2"
        ...
        <p
          ref={titleRef}
          className="text-[11px] font-semibold tracking-[0.22em] uppercase text-violet-300 mb-3"
        >
  ...
  return (
    <div className="absolute inset-0 flex items-end pb-20 px-8 md:px-16 lg:px-24 z-10 pointer-events-none">
      <div className="pointer-events-auto max-w-xl">
        ...
        <h1 ref={nameRef} className="text-5xl md:text-6xl lg:text-7xl font-black tracking-tight text-white mb-3 leading-none" style={{ textShadow: "0 2px 30px rgba(0,0,0,0.6)" }}>
          Kumail<br />
          <span className="bg-gradient-to-r from-violet-400 to-indigo-400 bg-clip-text text-transparent">Raza</span>
        </h1>
        <p ref={titleRef} className="text-base md:text-lg font-semibold tracking-widest uppercase text-violet-300 mb-3">Full Stack Developer</p>
        ...
        <div ref={ctaRef} className="flex gap-3 flex-wrap">
          <button onClick={() => scrollTo("projects")} className="px-6 py-3 bg-violet-600 hover:bg-violet-500 text-white text-sm font-semibold rounded-full transition-all duration-300 hover:scale-105 hover:shadow-lg hover:shadow-violet-500/40">View Work</button>
          <button onClick={() => scrollTo("contact")} className="px-6 py-3 bg-white/10 hover:bg-white/15 text-white text-sm font-semibold rounded-full border border-white/20 backdrop-blur-sm transition-all duration-300 hover:scale-105">Get in Touch</button>
        </div>
      </div>
    </div>
  );
